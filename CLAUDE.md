...
Key ROS2 parameters:
- `object_detection_new`: `service_name`, `model_path`, camera topics, `sort_mode`. FFS depth: `prefer_ffs` (default `true` — try FoundationStereo `~/get_depth` before native aligned depth on realsense; flip to `false` to use native only), `ffs_service` (default `'~/get_depth'`), plus `ffs_wait_for_service_s` / `ffs_call_timeout_s` / `ffs_align_to_color` / `ffs_fallback_log_period_s` (timeouts and throttled-warning period). Config in `src/object_detection_new/config/default.yaml`; same six params mirrored in `src/object_detection_generalist/config/default.yaml`.
