...
`tensorrt-cu12==10.16.1.11` (+ `-bindings`/`-libs`) is installed in
`.venv-fs`; the two-stage FP16 engines for the `23-36-37`
Fast-FoundationStereo checkpoint are built locally (GPU/TensorRT-version
locked, not shipped) via `python -m
foundation_stereo.scripts.build_trt_engines` — see
`src/foundation_stereo/README.md` § Weights & engines.
...
- **Additive install, zero version churn.** `torchreid==0.2.5` (PyPI wheel),
  ...
  **tensorboard 2.20.0 / protobuf 6.33.6 (2026-08-22):** the venv originally
  landed on tensorboard's early-2.20-era `protobuf` pin only implicitly
  (tensorboard 2.11.2 pinned `protobuf==3.20.3`); a dedicated `--no-deps`
  bump — `pip install --no-deps tensorboard==2.20.0
  tensorboard-data-server==0.7.2 protobuf==6.33.6` — moved exactly those
  three lines, `pip check` output unchanged, and the
  2.11-era orphans (`google-auth`, `google-auth-oauthlib`,
  `requests-oauthlib`, `tensorboard-plugin-wit`) were left in place
  (same policy as the jax orphans). See `DEV_NOTES.md`'s 2026-08-22 entry
  for the verbatim diff and the `torchreid`/`vision_track` regression gate.
...
# LLM-backed (kimi_api)
ros2 run kimi_api feature_recognition                # actions /feature_extraction_service, /seat_recommend_service
ros2 run kimi_api feature_matching                   # action /feature_matching_service
ros2 run kimi_api grocery_categorize                 # action /grocery_categorize
ros2 run kimi_api seat_recommend_bbox                # action /seat_recommend_bbox_service
ros2 run kimi_api object_scan                        # action /object_scan
ros2 run tk_vision_specialized waving_person_server  # action /detect_waving_persons
...
├── object_detection_generalist/   # Pretrained YOLO + YOLO-World (default fallback) or Gemini (default from VISION_VLM_FLASH_MODEL, enable_vlm) + MobileSAM mask, tk26 srv
...
└── foundation_stereo/             # FoundationStereo + Fast-FoundationStereo service/action + streaming depth publisher; lives in its own venv `.venv-fs` (torch 2.8 + cu128 + tensorrt-cu12 10.16.1.11) because those versions conflict with the shared `.venv-vision-main`. TensorRT FP16 engines for the `23-36-37` Fast-FoundationStereo checkpoint are GPU/TRT-version-locked and built locally via `build_trt_engines` (`src/foundation_stereo/README.md` § Weights & engines), not shipped. IR1→color alignment goes through `color_align_rs2.RealsenseAligner` (rs.align via software_device) — dense sub-pixel-splatted output. The naive forward-warp lives on as `color_align_legacy.reproject_ir_to_color` for non-RealSense sources; consumers of legacy output must hole-fill (medianBlur/dilate). See `debug_renders/2026-05-25-fs-vs-native-alignment/TRIAGE_FINDINGS.md` for the rationale.
```
...
- `kimi_api/*`: `llm_model`, `detection_service`, `log_prompts`. `feature_recognition`, `feature_matching`, and `grocery_categorize` each run a Gemini(`llm_model`)-primary -> Qwen(DashScope) provider-fallback chain: `vlm_fallback_provider` (default `'qwen'`, `''` disables; dropped with a warning when `DASHSCOPE_API_KEY` is absent — no crash) + a per-node Qwen model param (`feature_model_qwen` / `match_model_qwen` / `categorize_model_qwen`, all default from `VISION_QWEN_MODEL`), plus `vlm_timeout_s` / `vlm_max_retries` per provider. `grocery_categorize`'s `vlm_timeout_s` defaults to **60 s** (not 20): its default gemini-2.5-pro two-image call measures median 15.5 s / p90 28.4 s in seat_bench, so a 20 s cap would cancel ~1/3 of calls. Chain semantics everywhere: a legitimate answer (including "none"/empty) is terminal; only errors/exhaustion fall through; total failure raises and is reported as `error_msg='VLM ... failed on every provider: ...'` rather than a semantic result.
- `kimi_api/seat_recommend_bbox`: `vlm_strategy` (default `'bbox_select'` — one structured Qwen3-VL call returns a cushion box + occupancy per seat + the chosen empty seat, the 2026-06-02 benchmark winner; set `'point'` for the legacy Gemini-pointing path via `_seat_vlm.request_seat_chain` — Gemini-primary on `llm_model` with an automatic Qwen(`bbox_model_qwen`) fallback when `vlm_fallback_provider` is non-empty and `DASHSCOPE_API_KEY` is present). `vlm_provider` (default `'qwen'`) + `vlm_fallback_provider` (default `'gemini'`, `''` to disable) define the bbox_select fallback chain; `bbox_model_qwen` (default from `VISION_QWEN_MODEL`) / `bbox_model_gemini` (default from `VISION_VLM_MODEL`). `snap_enabled` now defaults to **`false`** — the chosen box already localizes the cushion, so its centre seeds the 5-tier robust-depth resolver directly (re-enable with `-p snap_enabled:=true` for the point path / noisy depth). Keys: bbox_select needs `DASHSCOPE_API_KEY` (or the typo'd `DASHCOPE_API_KEY`) for Qwen + `OPENROUTER_API_KEY` for the Gemini fallback; the point path needs only `OPENROUTER_API_KEY` (its Qwen fallback arms itself only when the DashScope key is also present). A legitimate "no empty seat" from the primary provider does **not** trigger fallback (only errors do). Benchmark + rationale: `src/kimi_api/seat_bench/report.md`.
...
- `waving_person_server` (`detect_waving_persons` action): `waving_detector` (default `'vlm'`) selects the waver source — `'vlm'` (VLM is the **sole** decider; MediaPipe pose skipped; YOLO still runs for the person mask → 3D centroid), `'hybrid'` (MediaPipe + VLM augmentation gated by `min_waving_persons`, the legacy 2026-07-03 behavior), or `'mediapipe'` (CV only, VLM never called). `'vlm'`/`'hybrid'` **auto-degrade to `'mediapipe'` at call time** when the VLM chain is empty (no key) or `enable_vlm_fallback=false` — so offline/no-key boxes (T1/T2 smoke) behave exactly as before; only key-present boxes are truly VLM-only. The VLM prompt requires a **live, physically-present** person — figures printed/displayed on a wall mural, ad, poster, screen, photo, etc. are rejected even if their pose looks like a wave (`_waving_vlm.py _SYSTEM_PROMPT`, prompt-only). `min_waving_persons` (`DetectWaving.action` goal field, default `0`) no longer gates whether the VLM runs in `'vlm'` mode (it always runs); it still gates in `'hybrid'`. `enable_vlm_fallback` (default `true`, global kill-switch), `vlm_provider` (`qwen`) → `vlm_fallback_provider` (`gemini`, `''` disables) errors-only chain; `vlm_model_qwen` (default from `VISION_QWEN_MODEL`) / `vlm_model_gemini` (default from `VISION_VLM_MODEL`); `vlm_timeout_s` (20.0), `vlm_max_retries` (3), `vlm_dedup_iou` (0.3). Pose backend (`hybrid`/`mediapipe` modes): `pose_model_path` (default `'pose_landmarker_full.task'`, resolved via `weights_cache.find_cached` — **no auto-download**, stage it with `scripts/download_models.py`) and `pose_delegate` (`'gpu'` default, `'cpu'` to force). MediaPipe **1.0.1 Tasks API** (`_pose_backend.py`); the legacy `mp.solutions` API no longer exists upstream. GPU delegate: ~8 ms/person, ~60 MiB VRAM, 3–6 s one-time init, falls back to CPU (~60 ms/person) with a WARN. Parity with the 0.10.9 behaviour is enforced by `test/test_pose_parity.py` against `test/fixtures/pose_parity/`. Keys: `DASHSCOPE_API_KEY`/`DASHCOPE_API_KEY` (qwen) or `OPENROUTER_API_KEY` (gemini), resolved via `_waving_vlm.py` (no `kimi_api` import; same decoupled convention as `vlm_match_client.py`). VLM-found wavers reuse an overlapping YOLO mask or box-center robust depth for their 3D centroid (`_waving_geometry.py`), are deduped against any MediaPipe wavers, drawn in the debug overlay as `waving (vlm)`, and tagged `waving_person_vlm` in the vision log. **Tradeoff:** `'vlm'` mode waits ~5–20 s/goal (VLM) vs ~10–50 ms/goal (MediaPipe on GPU, YOLO + N × ~8 ms pose; ~100–300 ms on CPU); set `waving_detector:=hybrid`/`mediapipe` for a fast path. Design: `docs/superpowers/specs/2026-07-04-waving-vlm-only-live-person-design.md` (extends `2026-06-02-waving-vlm-fallback-design.md`).
...
6. **Apply (per-robot, one command).** With `ROBOT_NAME` exported (refused when unset), run `python -m pan_tilt.calibration.apply_to_urdf --results calib_out/polish.json` (optional `--basic-root <tk25_basic root>`, `--allow-flipped-camera`). It atomically writes exactly **two per-robot files** in the tk25_basic source tree — `robots/$ROBOT_NAME/pan_tilt/pan_tilt_overrides.xacro` (mount geometry; the macro auto-includes it at xacro-parse time) + `offsets.yaml` (runtime joint offsets) — in lockstep with `.old-<ts>` backups. The shared `tinker_urdf`/`pan_tilt` xacros are never written, so tinker1/tinker2 calibrations can't overwrite each other. Then `tkbuild tk25_basic --packages-select tinker_robot_config` and relaunch robot_state_publisher + pan_tilt state_publisher. calib_web's Preview/Apply buttons drive the same code path (Preview renders unified diffs of both files, no writes).
