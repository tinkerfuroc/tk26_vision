...
def seed_handeye_override_xacro(joint_name, xyz_str, rpy_str):
    """Build a minimal per-robot wrist_camera.xacro override body.

    Returns a complete xacro file with one ``<joint>`` block whose ``<origin>``
    carries the calibrated values. The header comment instructs the operator
    on the one-time main-xacro include + vendor-block disable; the actual
    include wiring is out of scope for the Promote action (see README's
    "Per-robot xacro override (one-time setup)" section).
    """
    return _OVERRIDE_TEMPLATE.format(
        joint_name=joint_name, xyz_str=xyz_str, rpy_str=rpy_str)
