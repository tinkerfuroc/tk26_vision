...
| `orbbec_depth_topic` | `/camera/depth/image_raw` | Registered depth `Image` aligned to color (inherited from `YOLOSegmentationNode`; not overridden here). Must match what the camera launch publishes. Requires `depth_registration:=true` on `femto_bolt.launch.py`; `enable_colored_point_cloud` is no longer needed by this node. |
...
- **2026-05-28** — FFS-first depth with native fallback (`prefer_ffs`). Six new ROS params inherited from the `YOLOSegmentationNode` base: `prefer_ffs` (default `true`), `ffs_service` (default `'/foundation_stereo/get_depth'`), `ffs_wait_for_service_s` (0.2), `ffs_call_timeout_s` (8.0), `ffs_align_to_color` (true), `ffs_fallback_log_period_s` (30.0). When `prefer_ffs=true` (default), realsense detections try the FoundationStereo on-demand depth service first and fall back silently to native aligned depth on any failure. Vision_log sidecar JSON gains `depth_source ∈ {'ffs', 'native'}` per call. No srv/msg schema changes. Orbbec branch unchanged. Rollback: `ros2 param set /object_detection_generalist prefer_ffs false` (no restart needed).
