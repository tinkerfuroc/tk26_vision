...
def objective(params, data, L_pan, L_tilt):
    ...
    for sample in data:
        ...
        T_pred = forward_kinematics_fixed(theta_pan, theta_tilt, params, L_pan, L_tilt)
...
def print_results(res):
    ...
    print(f"  - Rotation (Euler xyz) [deg]: {np.round(rot0_euler_deg, 4)}")
    print(f"  - Rotation (Quaternion xyzw): {np.round(rot0_quat, 8)}")
    ...
    print(f"  - Rotation (Quaternion xyzw): {np.round(rot2_quat, 8)}")
    print("------------------------------------")

def main():
    L_pan = 0.135  # in meters, from blueprint (56mm)
    L_tilt = 0.040  # in meters, from blueprint (65.82mm)
    ...
    print_results(res)
    np.save("opt_params_fixed_pan_tilt.npy", res.x)

if __name__ == "__main__":
    main()
    print("Optimization complete. Parameters saved to opt_params_fixed_pan_tilt.npy.")
