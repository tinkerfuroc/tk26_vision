...
- **2026-06-14** — tracker now drives the head pan-tilt to keep the locked person
  ...
  benign without a servo — no `/pan_tilt_controller/state` ⇒ a throttled warning
  and zero commands; set `-p enable_pan_tilt_follow:=false` to disable (e.g. when
  another node owns the head). Hardware: flip `pan_sign` if the head turns away
  instead of toward the person.
